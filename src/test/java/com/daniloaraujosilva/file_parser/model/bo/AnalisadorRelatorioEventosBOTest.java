...
public class AnalisadorRelatorioEventosBOTest extends AbstractTest {
	...
	public void totalDeEventosDoCliente0001() {
		assertEquals(
			7,
			analisador.getTotalEventosCliente().get("0001"),
			0
		);
	}
	...
	public void totalDeEventosDoCliente0003() {
		assertEquals(
			3,
			analisador.getTotalEventosCliente().get("0003"),
			0
		);
	}
	...
	public void tempoMedioDeAtendimentoEmSegundosDoAtendenteAT01() {
		assertEquals(
			159,
			analisador.getTempoMedioAtendimentoAtendente().get("AT01"),
			0
		);
	}
	...
	public void tempoMedioDeAtendimentoEmSegundosDoAtendenteAT02() {
		assertEquals(
			156,
			analisador.getTempoMedioAtendimentoAtendente().get("AT02"),
			0
		);
	}
	...
	public void tipoComMaisEventos() {
		assertArrayEquals(
			new TipoEventoEnum[] {
				TipoEventoEnum.ALARME,
				TipoEventoEnum.DESARME,
				TipoEventoEnum.TESTE,
				TipoEventoEnum.ARME
			},
			analisador.getTiposOrdenadosNumerosEventosDecrescente().toArray(
				new TipoEventoEnum[TipoEventoEnum.values().length]
			)
		);
	}
